...
teacher.load_state_dict(torch.load('D:/AProject/meta/train_mata/teacher_model/teacher_model_144.pth',map_location = device))
#更改为新训练的教师网络
student = TransformNet(32).to(device)
student.load_state_dict(torch.load('D:/AProject/meta/train_mata/teacher_model/teacher_model1_100.pth',map_location = device))
...
class CNN_train():
    ...
    def __call__(self, cgp, gpuID, epoch_num=100, gpu_num=1):
        ...
        teacher_model.load_state_dict((torch.load('D:/AProject/meta/train_mata/student_model_t1_100.pth', map_location=device)))
        if gpu_num > 1:
        ...
        for epoch in range(1, epoch_num + 1):
            ...
            for input, target in train_bar:  # enumerate(self.dataloader):
                if target.size(1) == 3:
                    ...
                    teacher_recons_loss = (
                            F.mse_loss(teacher_yy, student_yy) + F.mse_loss(teacher_yyy, student_yyy) + F.mse_loss(teacher_yyyy, student_yyyy) +
                            F.mse_loss(teacher_yyyyy, student_yyyyy) + F.mse_loss(teacher_yyyyyy, student_yyyyyy))  # + F.mse_loss(de_f6, src_f6)

                    cont_loss = C2R(ablation=False).cuda(gpuID)
                    out = out.cuda()
                    input1 = input.cuda()
                    target1 = target.cuda()
                    cont = cont_loss(rec_hazy_free,target1,input1,out,0.8,0.2)

                    loss = contrastive_loss(rec_hazy_free, target1)
                    # loss = l1_loss + 0.04 * fe_loss + 0.04 * perceptual_loss + teacher_recons_loss
                    loss = l1_loss + 0.04 * perceptual_loss + 0.04 * teacher_recons_loss + 0.04 * cont + 0.001 * loss
                    ...
                    if ite % 80 == 0:
                        # print(output)
                        vutils.save_image(lr_patch.data, 'D:/AProject/meta/train_mata/student1_result/input_result/input_sample%d.png' % gpuID, normalize=True)
                        vutils.save_image(hr_patch.data, 'D:/AProject/meta/train_mata/student1_result/target_result/target_sample%d.png' % gpuID, normalize=True)  # True
                        vutils.save_image(rec_hazy_free.data, 'D:/AProject/meta/train_mata/student1_result/output_result/output_sample%d.png' % gpuID, normalize=True)
                        # print(output)

            print('Train set : Average loss: {:.4f}'.format(train_loss))
            print('time ', time.time() - start_time)
            torch.save(teacher_model.state_dict(), 'D:/AProject/meta/train_mata/student1_model/student_model_%d.pth' % int(epoch))
            ...
            if epoch % test_interval == 0:
                with torch.no_grad():
                    ...
                    for i, (input, target) in enumerate(self.val_loader):
                        ...
                        # save images
                        vutils.save_image(rec_hazy_free.data, 'D:/AProject/meta/train_mata/student1_result_t/output_result/%05d.png' % (int(i)), padding=0,
                                          normalize=True)  # False
                        vutils.save_image(lr_patch.data, 'D:/AProject/meta/train_mata/student1_result_t/input_result/%05d.png' % (int(i)), padding=0,
                                          normalize=True)
                        vutils.save_image(hr_patch.data, 'D:/AProject/meta/train_mata/student1_result_t/target_result/%05d.png' % (int(i)), padding=0,
                                          normalize=True)
                    ...
                    f.close()
                    print('------------------------')
                torch.save(teacher_model.state_dict(), 'D:/AProject/meta/train_mata/student1_model/student_model_t1_%d.pth' % int(epoch))
