...
from validata import validate, save_image_results
import utils
from options import *
np.random.seed(0)
from kt_data import DataGeneratorPaired
...
def main():
    ...
    # path_dataset = 'C:/Users/86198/PycharmProjects/pythonProject/myself/meta/train_mata/indoor/train'
    # path_aux = config['path_aux']
    path_aux = 'C:/Users/86198/PycharmProjects/pythonProject/myself/meta/train_mata/logs_checkpoints'
    ...
    #                                         hazy_dir_name=args.hazy_dir, gt_dir_name=args.gt_dir)
    train_data = TrainDatasetFromFolder1(
        'C:/Users/86198/PycharmProjects/pythonProject/myself/meta/train_mata/indoor/train/haze',
        'C:/Users/86198/PycharmProjects/pythonProject/myself/meta/train_mata/indoor/train/clear',
        crop_size=256)
    val_data = TestDatasetFromFolder(
        'C:/Users/86198/PycharmProjects/pythonProject/myself/meta/train_mata/indoor/test')
    test_data = TestDatasetFromFolder(
        'C:/Users/86198/PycharmProjects/pythonProject/myself/meta/train_mata/indoor/test')
    ...
    distilled_model = TransformNet(32).to(device)
    bestmodel_path = 'C:/Users/86198/PycharmProjects/pythonProject/myself/meta/train_mata/teacher_model'
    ...
    if (args.load_best_model == True and args.best_model_path != "" and os.path.exists(args.best_model_path)):
        ...
        model_checkpoint = torch.load(os.path.join(bestmodel_path, args.best_model_path))
        distilled_model.load_state_dict(model_checkpoint["state_dict"])
        print("Done")
    elif (args.load_best_teacher_model == True and args.best_teacher_model_path != "" and os.path.exists(
            args.best_teacher_model_path)):
        ...
        for epoch in range(args.epochs):
            ...
            if dehazing_loss < best_dehazing_loss:
                ...
                utils.save_checkpoint(
                    {'epoch': epoch + 1, 'state_dict': distilled_model.state_dict(), 'best_dehazing_loss':
                        best_dehazing_loss, "PSNR": valid_loss["loss_psnr"].avg, "SSIM": valid_loss["loss_ssim"].avg},
                    directory=path_cp)
            else:
    ...
    if os.path.isfile(best_model_file):
        ...
        print("Loaded best model '{0}' (epoch {1}; Dehaze Network Loss {2:.4f}, PSNR = {3:.4f}, SSIM = {4:.4f})".format(
            best_model_file, epoch, best_dehazing_loss,
            psnr_loss, ssim_loss))
        print('***Test***')
        valid_loss = validate(test_loader, distilled_model, epoch, args)

        print('Results on test set: Dehaze Network Loss = {0:.4f}, PSNR = {1:.4f}, SSIM = {2:.4f}'.format(
            valid_loss['dehazing_loss'].avg,
...
def train(train_loader, distilled_model, epoch, args):
    ...
    for i, (input ,target) in enumerate(tqdm(train_loader)):
        ...
        gt = Variable(input, requires_grad=False).cuda()
        hazy = Variable(target, requires_grad=False).cuda()

        # if torch.cuda.is_available():
        #     gt, hazy = gt.cuda(), hazy.cuda()

        loss = Distilled.backward(gt, hazy, args)
        ...
        if (i + 1) % args.log_interval == 0:
            print('[Train] Epoch: [{0}][{1}/{2}]\t'
                  ...
                  .format(epoch + 1, i + 1, len(train_loader), batch_time=batch_time, loss_teacher=loss_teacher_rec,
                          loss_student=loss_student_rec, loss_perc=loss_student_perceptual,
