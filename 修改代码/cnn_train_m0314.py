    ...
    def __init__(self, dataset_name, imgSize=256, batchsize=8):
        ...
        if dataset_name == 'mix' or dataset_name == 'yourdata':
            if dataset_name == 'mix':
                ...
                train_set = TrainDatasetFromFolder1('D:/AProject/meta/train_mata/io-haze/train/haze',
                                                    'D:/AProject/meta/train_mata/io-haze/train/clear', crop_size=self.imgSize)
                # test_set = TestDatasetFromFolder1(
                #     '/home/omnisky/4t/JTY/testdataset/indoor')
                test_set = TestDatasetFromFolder1(
                    'D:/AProject/meta/train_mata/io-haze/test')
        ...
        for epoch in range(1, epoch_num + 1):
            ...
            for input, target in train_bar:  # enumerate(self.dataloader):
                if target.size(1) == 3:
                    ...
                    if ite % 100 == 0:
                        # print(output)
                        vutils.save_image(lr_patch.data, 'D:/AProject/meta/results/Inputs/input_sample%d.png' % gpuID, normalize=True)
                        vutils.save_image(hr_patch.data, 'D:/AProject/meta/results/Targets/target_sample%d.png' % gpuID, normalize=True)  # True
                        vutils.save_image(output.data, 'D:/AProject/meta/results/Outputs/output_sample%d.png' % gpuID, normalize=True)
                        # print(output)

            print('Train set : Average loss: {:.4f}'.format(train_loss))
            print('time ', time.time() - start_time)
            #---------------------用以保存每轮次参数------------------------
            torch.save(model.state_dict(), 'D:/AProject/meta/results/Model/model_%d.pth' % int(epoch))
            ...
            if epoch % test_interval == 0:
                with torch.no_grad():
                    ...
                    for i, (input, target) in enumerate(self.val_loader):
                        ...
                        # print(output)
                        # save images
                        vutils.save_image(output.data, 'D:/AProject/meta/results_r/Outputs/%05d.png' % (int(i)), padding=0,
                                          normalize=True)  # False
                        vutils.save_image(lr_patch.data, 'D:/AProject/meta/results_r/Inputs/%05d.png' % (int(i)), padding=0,
                                          normalize=True)
                        vutils.save_image(hr_patch.data, 'D:/AProject/meta/results_r/Targets/%05d.png' % (int(i)), padding=0,
                                          normalize=True)
                    ...
                    f.close()
                    print('------------------------')
                torch.save(model.state_dict(), 'D:/AProject/meta/results_r/Model/model_%d.pth' % int(epoch))
